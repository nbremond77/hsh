...
void loop() {
  ...
  float pressureKPA, pressureSum, temperatureC, tempSum;
  float pressureInHg, convertedAltitude;
  const float KPA2INHG = 0.296134; 
  float altitudeCorrection = 585;
  int samples = 100;

  pressureSum = 0;
  tempSum = 0;
  for (int i=0; i<samples; i++) {
    pressureSum += mpl115a2.getPressure();
    delay (10);
    activityLed(0);
    tempSum += mpl115a2.getTemperature();
    delay (10);
    activityLed(1);
  }
     
  pressureKPA = pressureSum/samples;
  temperatureC = tempSum/samples;
  float temperatureF = ((1.8*temperatureC)+32);
  pressureInHg = pressureKPA * KPA2INHG;
  convertedAltitude = ((29.92 - pressureInHg)/1.00) * 1000;
  convertedAltitude += altitudeCorrection;
    
  Serial.print("Temp (*C): "); Serial.print(temperatureC, 2); Serial.println(" *C");
  Serial.print("Temp (*F): "); Serial.print((1.8*temperatureC)+32, 2); Serial.println(" *F");
  Serial.print("Pressure (kPa):  "); Serial.print(pressureKPA, 4); Serial.println(" kPa");
  Serial.print("Pressure (inHg): "); Serial.print(pressureInHg, 4); Serial.println(" inHg");
  Serial.print("Altitude (ft, tentative): "); Serial.print(convertedAltitude, 0); Serial.println(" ft\n");
    ...
    char converted_temp[] = "                                ";
    Serial.print(" and now for proper printing...");
    Serial.print(dtostrf((double)tempF, 6, 2, converted_temp));
    Serial.println("F");
    int sl = 0;
    char payload[64] = "                                                               ";
#ifdef RECV_MODE
    ...
    sprintf(&payload[strlen(payload)], ",%sF", converted_temp);
    Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);

    //converted_temp[20] = "                    ";
    dtostrf((double)tmp101, 6, 2, converted_temp);
    sprintf(&payload[strlen(payload)], ",%sF", converted_temp);
    Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);    
    //converted_temp[] = "                                ";
    dtostrf((double)temperatureF, 6, 2, converted_temp);
    sprintf(&payload[strlen(payload)], ",%sF", converted_temp);
    Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    //converted_temp[] = "                                ";

    dtostrf((double)convertedAltitude, 5, 0, converted_temp);
    sprintf(&payload[strlen(payload)], ",%sft", converted_temp);
    Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    //converted_temp[] = "                                ";


    int rh100 = relativeHumidity;
    sprintf(&payload[strlen(payload)], ",%d%%", rh100);
    Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    sprintf(&payload[strlen(payload)], ",%dmV", batteryMillivolts);
        Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    sprintf(&payload[strlen(payload)], ",%dmV", solarMillivolts);
        Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    sprintf(&payload[strlen(payload)], ",%c", chargeStat);
        Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    sprintf(&payload[strlen(payload)], ",cpo:%d%%", cpr);
        Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    
    sprintf(&payload[strlen(payload)], "\0");
        Serial.print("strlen "); Serial.print(sl++); Serial.print("="); Serial.println(strlen(payload),DEC);
    
    
    /*int n=sprintf (payload, "%d,%d.%02d,%d.%02d,%f,%f,%f,%f,%d%%,%dmV,%dmV,%c,cpo=%d ",
                              seq, tempF100/100, tempF100%100, tmp101100/100, tmp101100%100,
                              temperatureF, pressureKPA, pressureInHg, convertedAltitude,
                              rh100/100,
...
#ifndef RECV_MODE
    rf12_sendStart(header, payload, strlen(payload));
    activityLed(0);
    delay(20);
    activityLed(1);
  ...
  if (seq >=10) sleepWithBeacon(50*5);  //(26 = ~60 seconds between shots)
#endif
